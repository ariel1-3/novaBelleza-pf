...
if (isset($_POST['dounzip'])) {
  // verifica si el archivo para descomprimir es correcto
...
class Unzipper
{
  ...
  {
    // modifica los archivos rar y zip y los vuelve ducomento
  ...
  public function prepareExtraction($archive, $destination = '')
  {
    ...
    // solo extrae archivos existentes
  ...
  {
    // verifica que el servidor admita descomprimir 
    ...
    // verifica que el archivo puede ser almacenado y leido por el sistema
  ...
  {
    // verifica si el zip esta habilitado
    ...
    if (file_exists($destination . '/' . $filename)) {
      ...
      // valida el archivo tar y lo extrae
      if (pathinfo($destination . '/' . $filename, PATHINFO_EXTENSION) == 'tar') {
        $phar = new PharData($destination . '/' . $filename);
        if ($phar->extractTo($destination)) {
          $GLOBALS['status'] = array('success' => 'Extracted tar.gz archive successfully.');
          // borra los archivos de tipo .tar.
  ...
  {
    // verifica que el servido admita descompresion de archivos .tar.
    if (!class_exists('RarArchive')) {
      $GLOBALS['status'] = array('error' => 'Error: Your PHP version does not support .rar archive functionality. <a class="info" href="http://php.net/manual/en/rar.installation.php" target="_blank">How to install RarArchive</a>');
      return;
    }
    // verifica que el archivo es legible 
    if ($rar = RarArchive::open($archive)) {
      // comprueba que el destino de las imagenes exista
      if (is_writeable($destination . '/')) {
...
class Zipper
{
  ...
  private static function folderToZip($folder, &$zipFile, $exclusiveLength)
  {
    ...
    while (FALSE !== $f = readdir($handle)) {
      // comprueba la ruta principal del archivo 
      if ($f != '.' && $f != '..' && $f != basename(__FILE__)) {
        $filePath = "$folder/$f";
        // elimina el nombre de la ruta del archivo antes de comprimirlo 
